...
class mpu9250(object):
	def __init__(self, bus=1, address=MPU9250_ADDRESS):
		"""
		Setup the IMU
		...
		self.write(self.address, PWR_MGMT_1, 0x00)  # turn sleep mode off
		sleep(0.2)
		...
		# sudo i2cdetect -y 1
		self.write(self.address, INT_PIN_CFG, 0x22)
		self.write(self.address, INT_ENABLE, 0x01)
		sleep(0.1)
	...
	@property
	def accel(self):
		return self.read_xyz(self.address, ACCEL_DATA, self.alsb)

	@property
	def gyro(self):
		return self.read_xyz(self.address, GYRO_DATA, self.glsb)
	...
	def temp(self):
		...
		"""
		temp_out = self.read16(self.address, TEMP_DATA)
